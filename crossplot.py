...
class MyFrame(wx.Frame):
    ...
    def on_polygon_select(self, vertices):
        ...
        for df in self.dataframes:
            x = df[self.x_combo.GetValue()]
            y = df[self.y_combo.GetValue()]
            gr = df[self.gr_combo.GetValue()]
    ...
    def update_log_choices(self):
        self.log_choices = []

        for df in self.dataframes:
            self.log_choices.extend(list(df.columns))

        # self.log_choices = list(set(self.log_choices))  # Remove duplicates
